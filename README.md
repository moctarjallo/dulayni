...
`dulayni` is a CLI tool and Python library for building and running Retrieval-Augmented Generation (RAG) agents via MCP (Model Context Protocol) servers. It provides a simple interface to configure, launch, and interact with AI agents backed by OpenAI and custom tool servers.

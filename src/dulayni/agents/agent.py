...
class Agent:
    ...
    async def create(
        ...
        if debug_tools:
            tools = await cls.debug_tools(
                srv
            )  # tools from fastmcp aren't discovered by langchain, so we use it just for debugging for now
    ...
    async def debug_tools(cls, srv):
        ...
        async with Client(srv) as client:
            ...
            langchain_tools = await utils.convert_mcp_tools_to_langchain(
                fastmcp_tools, client
            )
